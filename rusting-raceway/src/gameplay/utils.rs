//! Contains gameplay utility functions
...
/// Determines the distance traveled along a path
pub fn determine_path_distance(points: &Vec<Vec3>) -> f32 {
    let mut distance = 0.0;
    for j in 0..points.len() - 1 {
        distance += points[j].distance(points[j + 1]);
    }
    distance
}
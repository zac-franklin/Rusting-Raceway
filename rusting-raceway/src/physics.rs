...
pub fn determine_track_points(length: f32, radius: f32, bend_sections: u32) -> Vec<Vec3> {
    ...
    // the number of sections in the curve. Since we start the loop at 0, we minus two
    // the end condition to avoid path backtracking
    for _ in (0..bend_sections - 2).step_by(2) {
